  ...
  <div id="centerRight1">
    <div class="bg-color-black">
      <div class="d-flex pt-2 pl-2">
        ...
        <div class="d-flex">
          <span class="fs-xl text mx-2" style="font-size:17px">电车油车对比</span>
          <a v-on:click="oilClick" herf="#"><span style="color:deepskyblue">油车/</span></a>
          <a v-on:click="electricClick" herf="#"><span style="color:#33cea0">电车</span></a>
        </div>
      </div>
...
export default {
  ...
  methods:{
    stopScroll() {
      // 找到滚动板的定时器并清除
      const board = this.$el.querySelector('.dv-scroll-board')
      if (board) {
    ...
    async electricClick(){
        const res = await this.$http.get('myApp/centermostright/0/')
        this.$set(this.config,"data",res.data.realData)
    },
    async oilClick(){
        const res = await this.$http.get('myApp/centermostright/1/')
        this.$set(this.config,"data",res.data.realData)
    }
  },
  async mounted(){
